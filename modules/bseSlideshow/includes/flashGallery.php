...
// Here we get the data from the database.
    $showid = 1;
    $data = $bse->db->getQueryData("SELECT * FROM `bseFlashSlideshows` WHERE `id`='$showid'");
    $dur = $data['duration'];
...
// Here we get the images that are in the slide show
    $query = $bse->db->getQueryResult("SELECT * FROM `bseFlashSlideshowImages` WHERE `show_id`='$showid' ORDER BY `sorder` ASC");
    while( $data = mysql_fetch_array($query) )
    {
        echo "<image label=\"{$data['label']}\" file=\"../images/{$data['file']}\" link=\"{$data['link']}\" />
        ";
    }
    echo "
    </images>

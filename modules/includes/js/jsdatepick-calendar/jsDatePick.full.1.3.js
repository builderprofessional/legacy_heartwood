	...
	if (this.oConfiguration.useMode == 2){
		if (this.oConfiguration.target != false){
			if (typeof(JsgetElem(this.oConfiguration.target)) != null){
				...
				aSpan.style.top = inputElement.offsetTop + "px";
				aSpan.style.left = inputElement.offsetLeft + "px";
	...
	while (oDay.getMonth() == cmpMonth){
		...
		if (this.isSelectedDay(oDay.getDate())){
			...
			if (parseInt(aDayDiv.getAttribute("isToday")) == 1){
				this.setC(aDayDiv, "dayDownToday " + this.oConfiguration.cellColorScheme + "Today");
			} else {
				this.setC(aDayDiv, "dayDown " + this.oConfiguration.cellColorScheme + "Today");
			}	
			...
			if (parseInt(aDayDiv.getAttribute("isToday")) == 1){
				if (disabledDayFlag){
					...
					this.setC(aDayDiv, "dayNormalToday " + this.oConfiguration.cellColorScheme + "Today");
				}
			} else {
				if (disabledDayFlag){
					this.setC(aDayDiv, "dayDisabled");
				} else {
					this.setC(aDayDiv, "dayNormal " + this.oConfiguration.cellColorScheme);
				}
			}
		}

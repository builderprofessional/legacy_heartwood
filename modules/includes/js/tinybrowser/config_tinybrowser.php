...
$tinybrowser['obfuscate'] = 'foiamflaoaijdf0u0ja3ljk23fp9';
...
$tinybrowser['docroot'] = getModulesParentDir( dirname(__FILE__) ); //rtrim($_SERVER['DOCUMENT_ROOT'],'/');
$rootPath = $tinybrowser['docroot'] . (substr($tinybrowser['docroot'], -1) == "/" ? "" : "/" );

chdir(dirname(__FILE__)); 
require_once($rootPath."modules/includes/main/bseMain.php");
...
// Folder to docroot on page that calls TB
$tinybrowser['pageRoot'] = $bse->pageRoot;

// File upload paths (set to absolute by default)
$tinybrowser['path']['image'] = $bse->content->getResourceDir(); // Image files location - also creates a '_thumbs' subdirectory within this path to hold the image thumbnails
$tinybrowser['path']['media'] = $bse->content->getResourceDir(); // Media files location
$tinybrowser['path']['file']  = $bse->content->getResourceDir(); // Other files location
...
$tinybrowser['link']['image'] = $bse->pageRoot . $tinybrowser['path']['image'];    //$tinybrowser['path']['image']; // Image links
$tinybrowser['link']['media'] = $bse->pageRoot . $tinybrowser['path']['media'];    //$tinybrowser['path']['media']; // Media links
$tinybrowser['link']['file']  = $bse->pageRoot . $tinybrowser['path']['file'];    //$tinybrowser['path']['file']; // Other file links
...
$tinybrowser['delayprocess'] = 0; // Value in seconds
    ...
    function getModulesParentDir($curDir, $resetRoot = true)
    {
        static $ret = "";
        if( $resetRoot )
            $ret = "";
        ...
        else if( basename($curDir) != "" && basename($curDir) != "/" && basename($curDir) != "." )
        {
            $ret .= "../";

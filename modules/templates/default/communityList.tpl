    <div id="communityLinks" class="communitiesList">
        <h3>Locations:</h3>
        <ol type="A">
<?php
    $count = 0;
    $href = "Javascript:map.setCenter(new GLatLng(%s), {$this->googleMap->zoom}); GEvent.trigger(markers[%s],\"click\");";
    ...
    {  ?>
            <li><a href='<?=( $this->user->loggedIn&&$this->user->backdoor ? sprintf($href, $comm->id) : sprintf($href, $comm->point, $count) )?>' title="<?= $comm->name ?>"><?= $comm->name ?></a>
                <div class="subText">
                    <a href="details.php?commid=<?= $comm->id ?>" title="<?= $comm->name.' - Home' ?>">Details</a> | 
                    <a href="localinfo.php?commid=<?= $comm->id ?>" title="<?= $comm->name.' - Local Info' ?>">Local Info & Directions</a>

...
//  This allows for pages in subdirectories and for this site to be housed in any directory under any site.
    ...
    $bse->setReturnPage($bse->curPage . ( isset($_SERVER['QUERY_STRING']) && @trim( $_SERVER['QUERY_STRING']) != "" ? "?{$_SERVER['QUERY_STRING']}" : "" ) );
...
//  Add the template directory, if specified
   ...
   // Get Variables
    ...
    $bse->dynamicSlideshow->extraStyle = "position:fixed; z-index:-1; top:0; left:0; border:solid 1px; ";
    $bse->dynamicSlideshow->autoStart = false;
    $bse->dynamicSlideshow->singleRandom = true;
    
   

    $bse->pageName = basename($_SERVER['PHP_SELF']);
